// /lib/ctaEngine.js
// TinmanApps — Psychographic CTA + Subtitle Engine v3.4 “Context-Anchored Precision”
// ───────────────────────────────────────────────────────────────────────────────
// Enhancements over v3.3:
// • Absolute per-page CTA uniqueness
// • Category-anchored objects (“AI workflows”, “marketing campaigns”, etc.)
// • Context-aware phrasing for both CTAs & subtitles
// • Natural adverb logic (no awkward “with seamlessly” phrases)
// • Retains full CTR weighting + semantic variation hooks
// ───────────────────────────────────────────────────────────────────────────────
...
export function createCtaEngine() {
  ...
  return {
    generate({ title = "", slug = "", cat = "software" }) {
      ...
      cta = cta.replace("{obj}", pick(objects));

      // Inject minor semantic verb variation
      if (Math.random() < 0.3) {
    ...
    generateSubtitle({ title = "", category = "software" }) {
      ...
      const set = SUB_TEMPLATES[cluster] || SUB_TEMPLATES[category] || SUB_TEMPLATES.software;
      let subtitle = pick(set);

      // Add tone descriptor occasionally
