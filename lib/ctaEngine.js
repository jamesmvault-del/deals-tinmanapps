// /lib/ctaEngine.js
// TinmanApps — Psychographic CTA + Subtitle Engine v4.1.2 “Grammar & Anchor Finalization”
// ───────────────────────────────────────────────────────────────────────────────
// Focus: Resolves “simplifies to” / “delivers to” errors, removes redundant anchors,
// enforces complete verb-object logic, restores natural CTA phrasing.
// ───────────────────────────────────────────────────────────────────────────────
...
function enforceVerbObjectLogic(cta) {
  ...
  if (!found) return `${verb} your ${pick(allowed)} →`;
  if (cta.includes("smarter")) cta = cta.replace("your smarter", "smarter");
  if (!cta.endsWith("→")) cta += " →";
  return cta.trim();
}
...
function smartJoin(sub) {
  // Repairs only known cases, prevents "simplifies to"
  return sub
    .replace(/\b(brings)\s+order\b/i, "brings order to")
    .replace(/\b(delivers)\s+clarity\b/i, "delivers clarity and performance")
    .replace(/\s{2,}/g, " ")
    .trim();
}
...
function anchorSubtitle(sub, cluster, cta, category) {
  ...
  const noun = anchor.split(" ").pop().replace(/\W/g, "");
  const lower = sub.toLowerCase();
  if (lower.includes(noun) || lower.match(/\b(across|for|in|through)\syour/i)) return sub;
