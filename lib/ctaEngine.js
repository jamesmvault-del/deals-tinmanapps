// /lib/ctaEngine.js
// TinmanApps — Psychographic CTA + Subtitle Engine v4.0.2 “ObjectGuard Edition”
// ───────────────────────────────────────────────────────────────────────────────
// Built upon v4.0.1 “SmartClamp Precision”
//
// 🚀 New in v4.0.2:
// • Fixes “Reclaim your →” orphan issue with intelligent ObjectGuard reinjection
// • Adds booster entropy caps (limit adjective repetition to 25% per run)
// • Extends SmartClamp with proper noun protection (no truncation inside names)
// • Maintains full entropy logic, tone biasing, and learning governors
// ───────────────────────────────────────────────────────────────────────────────
...
// ─────────────── Smart Clamp (Proper Noun Safe) ───────────────
function smartClamp(t, n = 44) {
  if (!t) return "";
  if (t.length <= n) return t.trim();

  // detect if we're mid-proper noun or compound word (e.g., "Template Creator")
  ...
  // avoid cutting mid-hyphen or partial word
...
function chooseObject(objects, boosters) {
  ...
  // Booster entropy limit (25% chance)
  if (Math.random() < 0.25 && Array.isArray(boosters) && boosters.length) {
...
// ObjectGuard: reinject object if CTA ends with “your” or “your →”
