...
// ───────────────────────────────────────────────────────────────────────────────
// TinmanApps — Feed Normalizer v4.5
// “Referral-Lock • Canonical-Slug • Context-Safe Description Edition”
//
// PURPOSE:
// • Strict Mode A (zero category overrides)
// • Repairs title → slug consistency deterministically
// • PRESERVES full description (context-aware enrichment ready)
// • Sanitises but NEVER truncates description text
// • NEVER modifies CTA/subtitle (delegated to ctaEngine)
...
// Sanitises description — removes HTML but preserves all readable context.
// Never truncates. Leaves natural sentence structure intact.
...
export function normalizeFeed(rawFeed = []) {
  ...
  const normalized = rawFeed
    .map((item) => {
      ...
      // 6️⃣ Description sanitisation — preserve full OG text, never truncate
      ...
      return {
        ...
        referralUrl,
        image,
        description, // ← preserved in full for contextual CTA generation
        seo,
        normalizedAt: now,
      };
    })
  ...
  console.log(`✅ [FeedNormalizer v4.5] Normalized ${normalized.length} entries (context-safe)`);
  return normalized;
}
