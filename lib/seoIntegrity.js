// /lib/seoIntegrity.js
// ───────────────────────────────────────────────────────────────────────────────
// TinmanApps — SEO Integrity Engine v4.6
// “Context-Enriched • Category-Adaptive • Product-Aware • Entropy-Safe”
//
// PURPOSE:
// • Guarantees SEO fields (cta, subtitle, clickbait, keywords) ALWAYS exist.
// • Expanded lexicons for AI/creative/software categories.
// • Relaxed product-name rule (only enforced for multi-word/6+ char titles).
// • Cleans CTA token duplication (“your your”, “with for better…”).
// • Adds contextual template diversity per category.
...
// ───────────────────────────────────────────────────────────────────────────────
// Category Lexicons & Hooks (expanded AI/creative coverage)
// ───────────────────────────────────────────────────────────────────────────────
const KEYWORDS = {
  ai: [
...
const HOOKS = {
  ai: ["Reinvent your workflow with AI", "Automate intelligently", "Build smarter with LLMs"],
...
const BENEFITS = [
  ...
  "reduce friction", "deliver consistently", "streamline workflows", "save time"
];

const SUB_VERBS = [
  "Streamlines", "Boosts", "Enhances", "Optimizes",
  "Accelerates", "Clarifies", "Improves", "Strengthens", "Empowers", "Simplifies"
];
const SUB_OBJECTS = [
  "workflow clarity", "daily operations", "team output",
  "creative flow", "project momentum", "system performance",
  "customer engagement", "AI capabilities", "design precision"
];
const SUB_ENDINGS = [
  "for measurable progress.", "so you save hours weekly.",
  "to remove unnecessary friction.", "so your results compound.",
  "to keep everything running smoothly.", "for better business outcomes."
];
...
function pickDet(seed, arr) {
  ...
  return arr[parseInt(sha1(seed).slice(0, 8), 16) % arr.length];
}
function multiPickDet(seed, arr, n) {
  ...
  for (let i = 0; i < Math.min(n, arr.length); i++) {
    let idx = parseInt(sha1(base + ":" + i).slice(0, 8), 16) % arr.length;
    let s = 0;
    while (used.has(idx) && s < arr.length) { idx = (idx + 1) % arr.length; s++; }
    used.add(idx);
    out.push(arr[idx]);
  }
  return out;
...
function dedupeTitle(t = "", title = "") {
  ...
  const out = t.split(/\s+/).filter((w) => !low.includes(w.toLowerCase())).join(" ");
  return out.trim() || t;
}
function validateCTA(t = "") {
  const s = normalizeSeams(t.trim());
...
// ───────────────────────────────────────────────────────────────────────────────
// Deterministic builders (category adaptive)
// ───────────────────────────────────────────────────────────────────────────────
function buildCTA(item, used) {
  const existing = item.seo?.cta?.trim?.();
  if (existing) {
    const fixed = validateCTA(existing);
    if (fixed) { used.add(fixed); return clamp(fixed, 64); }
  }
  ...
  if (used.has(cta)) {
    cta = `${pickDet(seed + ":v:alt", ["Level-up", "Strengthen", "Advance", "Upgrade"])} your ${pickDet(seed + ":o:alt", ["daily work", "team output", "delivery", "throughput"])} →`;
  }

  cta = normalizeSeams(cta);
  used.add(cta);
  return clamp(cta, 64);
}

function buildSubtitle(item, used) {
  const existing = item.seo?.subtitle?.trim?.();
  if (existing) {
    const norm = validateSubtitle(existing);
    if (norm) {
      const f = clamp(dedupeTitle(norm, item.title), 160);
      used.add(f);
      return f;
    }
  }
  ...
  let sub = `${v} ${o} ${e}`;
  if (used.has(sub)) {
    sub = `${pickDet(seed + ":v2", SUB_VERBS)} ${pickDet(seed + ":o2", SUB_OBJECTS)} ${pickDet(seed + ":e2", SUB_ENDINGS)}`;
  }

  sub = normalizeSeams(sub);
  const f = clamp(sub, 160);
  used.add(f);
  return f;
}

function buildClickbait(item, cat) {
  const seed = `cb:${item.slug || item.title}:${cat}`;
  const hook = pickDet(seed + ":h", HOOKS[cat] || HOOKS.software);
  const ben = pickDet(seed + ":b", BENEFITS);
  const title = item.title?.trim?.() || "";
  return clamp(`${hook} — ${title} helps you ${ben}`, 160);
}

function buildKeywords(item, cat) {
  return multiPickDet(`kw:${item.slug || item.title}:${cat}`, KEYWORDS[cat] || KEYWORDS.software, 3);
}

// ───────────────────────────────────────────────────────────────────────────────
// Integrity QA (context-aware relaxed rules)
// ───────────────────────────────────────────────────────────────────────────────
...
function validateSeoFields(item, category) {
  ...
  for (const banned of BANNED) {
    if (sub.toLowerCase().includes(banned)) errors.push("subtitle-banned-term:" + banned);
  }

  // Category lexicon presence (relaxed threshold)
  ...
  // Product name presence (only for multi-word or long names)
  const nameWords = title.split(/\s+/).filter((x) => x.length > 3);
  const longTitle = title.replace(/\s+/g, "").length >= 6;
  const enforceName = nameWords.length > 1 || longTitle;
  if (enforceName) {
    const hasProductName = nameWords.some((w) => sub.toLowerCase().includes(w.toLowerCase()));
    if (!hasProductName) errors.push("subtitle-missing-product-name");
  }
...
export function ensureSeoIntegrity(feed) {
  ...
  const updated = feed.map((item) => {
    ...
    const keywords =
      Array.isArray(item.seo?.keywords) && item.seo.keywords.length > 0
        ? item.seo.keywords
        : buildKeywords(item, cat);

    if (seenCTAs.has(cta)) problems.push(`dup-cta:${cta}`);
    if (seenSUBs.has(subtitle)) problems.push(`dup-sub:${subtitle}`);
    seenCTAs.add(cta);
    seenSUBs.add(subtitle);
  ...
  console.log(`✅ [SEO Integrity v4.6] ${updated.length} verified. Entropy CTA:${eCTA} Subtitle:${eSUB}`);
