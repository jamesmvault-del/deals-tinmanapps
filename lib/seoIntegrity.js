// /lib/seoIntegrity.js
// ───────────────────────────────────────────────────────────────────────────────
// TinmanApps — SEO Integrity Engine v2.2.2 “Category Safe-Fallback”
//
// Purpose:
// Expands entropy engine with universal category safety.
// Fixes "undefined length" errors by enforcing guaranteed fallbacks
// for unknown categories such as "software", "deals", or "apps".
// Retains full diversity, emotional mapping, and keyword entropy logic.
//
// Integration point:
...
const EMOTION_MAP = {
  ...
  software: ["optimize", "simplify", "automate", "streamline", "enhance", "refine", "elevate"],
};
...
// ─────────────── Helpers ───────────────
function rand(arr) {
  if (!Array.isArray(arr) || arr.length === 0) return "Discover what’s possible →";
...
function randomHook(cat) {
  const hooks = {
    ...
    software: ["Simplify smarter", "Discover your next breakthrough"],
  };
...
export function ensureSeoIntegrity(feed) {
  ...
  const updated = feed.map((item) => {
    ...
    const keywords = SEO_KEYWORDS[cat] || SEO_KEYWORDS.software;
    ...
    let ctaPool = CTA_VARIANTS[cat] || CTA_VARIANTS.software;
    let cta = item.seo?.cta || rand(ctaPool);
    let attempts = 0;
