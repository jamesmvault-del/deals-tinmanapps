// ───────────────────────────────────────────────────────────────────────────────
// TinmanApps Adaptive Feed Engine v4.5.1 — “Deferred Hydration Capture”
// • Headless-safe GraphQL intercept with live-traffic verification
// • Waits until ListingPageQuery payloads are seen before proceeding
// • Falls back cleanly to RSS or cache if no payload after timeout
// ───────────────────────────────────────────────────────────────────────────────
...
const HYDRATION_WAIT_MS = 40000;
const RETRY_LIMIT = 3;

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
function ensureDir(p) { if (!fs.existsSync(p)) fs.mkdirSync(p, { recursive: true }); }
function writeJson(f, d) { ensureDir(DATA_DIR); fs.writeFileSync(path.join(DATA_DIR, f), JSON.stringify(d, null, 2)); }
function readJsonSafe(f, fb = []) { try { return JSON.parse(fs.readFileSync(path.join(DATA_DIR, f), "utf8")); } catch { return fb; } }
function toSlugFromUrl(u) { const m = u.match(/\/products\/([^/]+)\//i); return m ? m[1] : null; }
function proxiedImage(s) { return `${SITE_ORIGIN}/api/image-proxy?src=${encodeURIComponent(s)}`; }
...
function normalizeRecord({ slug, title, url, cat, image }) {
  const safe = slug || toSlugFromUrl(url) || (title ? title.toLowerCase().replace(/\s+/g, "-") : "deal");
  return {
    title: title || safe,
    slug: safe,
    category: cat,
    url,
    referralUrl: trackedUrl({ slug: safe, cat, url }),
    image: image ? proxiedImage(image) : `${SITE_ORIGIN}/assets/placeholder.webp`,
    seo: {
      clickbait: `Discover ${title || safe} — #1 in ${cat}`,
      keywords: [cat, "AppSumo", "lifetime deal", safe, "exclusive offer"],
    },
  };
}
function extractOg(html) {
  const get = (p) => {
    const rx = new RegExp(`<meta[^>]+property=["']${p}["'][^>]+content=["']([^"']+)["']`, "i");
...
// ───────────────────────────────────────────────────────────────────────────────
// GraphQL intercept with hydration verification
// ───────────────────────────────────────────────────────────────────────────────
async function collectProductLinksViaIntercept(cat, listUrl) {
  ...
  page.on("response", async (res) => {
    ...
    if (url.includes("/graphql") && url.includes("ListingPageQuery")) {
      try {
        ...
        const nodes = json?.data?.listingPage?.listingsConnection?.edges?.map((e) => e.node) || [];
        for (const n of nodes) if (n?.url && n?.title) captured.add(`https://appsumo.com${n.url}`);
        hydrated = captured.size > 0;
      } catch {}
    }
  });
  ...
  while (Date.now() - start < HYDRATION_WAIT_MS && captured.size < 10) {
    await page.mouse.wheel({ deltaY: 1000 });
    await sleep(1500);
  }

  // give one last grace window for background fetches
  await sleep(2500);
  await browser.close();
...
async function fetchProductDetail(url, cat) {
  try {
    ...
    const og = extractOg(html); const slug = toSlugFromUrl(url);
...
async function withConcurrency(items, limit, worker) {
  ...
  const runners = new Array(Math.min(limit, items.length)).fill(0).map(async () => {
    while (true) { const idx = i++; if (idx >= items.length) return;
...
async function buildCategory(engine, cat, listUrl) {
  ...
  let links = [];
  try { links = await collectProductLinksViaIntercept(cat, listUrl); }
  catch (err) { console.warn(`  ⚠️ GraphQL intercept failed for ${cat}: ${err.message}`); }

  if (links.length === 0) {
    console.log(`  🧩 Using RSS fallback for ${cat}`);
    const rss = await fetchRssFallback(cat);
    if (rss.length) { writeJson(`appsumo-${cat}.json`, rss); console.log(`✅ Saved ${rss.length} → via RSS`); return; }
    console.log(`  ♻️ Using cached data for ${cat}`); links = readJsonSafe(`appsumo-${cat}.json`, []).map((r) => r.url);
  }

  const raw = await withConcurrency(links, DETAIL_CONCURRENCY, (u) => fetchProductDetail(u, cat));
  const clean = raw.map((r) => { const parts = (r.title || "").split(/\s*[-–—]\s*/); const brand = parts[0]?.trim() || r.slug; return { ...r, title: brand }; });
