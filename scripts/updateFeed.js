...
// ───────────────────────────────────────────────────────────────────────────────
// TinmanApps Adaptive Feed Engine v4.7 “Integrity Lock”
// ───────────────────────────────────────────────────────────────────────────────
// • GraphQL Intercept + DOM + Cache fusion (evergreen resilience)
// • Deduplication by slug hash (prevents recursive enrichment)
// • Compatible with CTA Engine + Insight Pulse + master-cron
// • Runs on GitHub or Render with zero local dependencies
// ───────────────────────────────────────────────────────────────────────────────
...
// ───────────────────────────────────────────────────────────────────────────────
// Puppeteer Intercept (GraphQL Capture)
// ───────────────────────────────────────────────────────────────────────────────
...
// Deduplicate by slug hash
function dedupeDeals(deals) {
  const seen = new Set();
  const clean = [];
  for (const d of deals) {
    const h = hashSlug(d.slug || d.title || "");
    if (!seen.has(h)) {
      seen.add(h);
      clean.push(d);
    }
...
async function buildCategory(engine, cat, listUrl) {
  ...
  if (links.length === 0) {
    ...
    if (rssRecords.length > 0) {
      const deduped = dedupeDeals(rssRecords);
      writeJson(`appsumo-${cat}.json`, deduped);
      console.log(`✅ Saved ${deduped.length} → via RSS fallback`);
      return;
    }
  ...
  const deduped = dedupeDeals(clean);
  const enriched = engine.enrichDeals(deduped, cat);
  ...
  console.log(`  ✅ Unique deals saved: ${enriched.length}`);
...
async function main() {
  ...
  for (const [cat, listUrl] of Object.entries(CATEGORY_URLS)) {
    try {
      ...
      console.error(`⚠️ Skipped ${cat}:`, err.message);
    }
  }
