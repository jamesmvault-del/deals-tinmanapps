// /api/categories.js
// TinmanApps — Category Renderer v10.0 “Active-Only • Deterministic • Engine-Synced SEO Renderer”
// ───────────────────────────────────────────────────────────────────────────────
// Alignment with updateFeed v10 & CTA Engine v10:
// • Only ACTIVE items render (archived hidden completely)
// • Structured data reflects ACTIVE items only
// • Deterministic order: category file order from updateFeed (no ranking)
// • Referral-safe masking via /api/track + REF_PREFIX
// • Hard 48-card visual cap (post-filter)
// • STRICT SUBTITLE MODE: always generated by CTA Engine (no scraped contamination)
// • CTA/subtitle lengths managed by engine — no local double-clamping
...
// STRICT: brand-only; never reuse scraped subtitle fragments
...
export default async function handler(req, res) {
  ...
  // ✅ Structured data reflects ACTIVE ONLY (flattened compliance)
  const breadcrumbLd = {
  ...
  // STRICT SUBTITLE MODE: ALWAYS engine-generated; never from scraped data
  // ─────────────────────────────────────────────────────────────────────────────
  const cards = deals
    .map((d) => {
      ...
      let subtitle = "";
      try {
        subtitle = engine.generateSubtitle({ title: brand, category: cat, slug });
      } catch {
        subtitle = "";
      }
      subtitle = dedupeByBrand(brand, subtitle);

      let cta = "";
      try {
        cta =
          (d.seo?.cta && d.seo.cta.trim()) ||
          engine.generate({ title: brand, cat, slug });
      } catch {
...
<footer>${escapeHtml(ARCHETYPE[cat])}</footer>
  ...
  console.log(`✅ [Category v10] ${cat} → ${activeCount} active • Engine:${CTA_ENGINE_VERSION}`);
}
